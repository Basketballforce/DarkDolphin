...
// throttle the amount of functions calls to moveSunMoon function using throttling
function throttle(delay, fn) {
  // define last call
  let lastCall = 0;
  return function () {
    const now = (new Date).getTime();
    // if the difference between the last call and now is less than the day just return. Don't call function
...
// add event listener to manage resize throttle calls
window.addEventListener("resize", throttle(1, moveSunMoon));
